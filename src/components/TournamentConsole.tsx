...
function Central({ players, activeScoreDay, setActiveScoreDay, activeRoundType, setActiveRoundType, skinPot, entriesFor, skinPayouts, skinResults, closestToPin, scrambleResults, scramblePayouts, teams, skinIsPosted, scrambleIsPosted, canEdit, skinScores, allClosestToPin, teamsByDay, scrambleTotals, postings, openSkins, openScramble }: {
  ...
  const roundAccessCopy = canEdit
    ? isSkins ? "Commissioner override: select a player to review or correct their complete 18-hole card." : "Commissioner override: select a team to review or correct its complete 18-hole scramble card."
    : isSkins ? "View any player's complete 18-hole card after the round is posted." : "View any team's complete 18-hole scramble card after the round is posted.";
  ...
  return <div className="space-y-6">
    ...
    <section className="club-hero grid gap-5 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-end">
      ...
      <div><p className="club-kicker">{capitalize(activeScoreDay)} score ledger</p><h2 className="club-display mt-3 text-4xl sm:text-6xl">{isSkins ? "Skins." : "Scramble."}</h2>{!isSkins ? <p className="mt-3 text-sm text-stone-300">Six team scorecards · first and second place payouts</p> : null}</div>
      <div className={`grid ${isSkins ? "grid-cols-2" : "grid-cols-3"} gap-2`}><Stat label="Status" value={isPosted ? "Posted" : "Draft"} detail="" />{!isSkins ? <Stat label="Cards" value={String(teams.length)} detail="teams" /> : null}<Stat label="Purse" value={`$${isSkins ? skinPot.total : scramblePot}`} detail="" /></div>
    </section>
...
function Setup({ activeDay, setActiveDay, players, setPlayers, teams, setTeams, resetAllTeams, locked, lifecycleBusy, runLifecycleAction }: { activeDay: ScrambleDay; setActiveDay: (day: ScrambleDay) => void; players: Player[]; setPlayers: React.Dispatch<React.SetStateAction<Player[]>>; teams: Team[]; setTeams: React.Dispatch<React.SetStateAction<Team[]>>; resetAllTeams: () => void; locked: boolean; lifecycleBusy: boolean; runLifecycleAction: (action: "seed-preview" | "start-tournament") => void }) {
  return <div className="space-y-6"><SectionTitle eyebrow="Commissioner workspace" title="Roster, bands & teams" text={locked ? "The tournament has started. The roster and both days of teams are locked; this page is now a read-only record of the field. Score entry is unaffected." : "Arrange the field and the teams, then start the tournament to lock them in and clear the board."} />
    ...
    <section className="club-card p-5 sm:p-7"><p className="club-kicker">The Classic pairings</p><h3 className="club-card-title mt-1">{capitalize(activeDay)} · six teams of four</h3><p className="mt-2 text-sm text-stone-300">{locked ? "These pairings are locked for the tournament. Changing them is a backend-only action." : "Friday and Saturday are separate. Teams have no names — they are simply who is on them. Swap any player below; selecting someone already assigned swaps the two positions."}</p><div className="mt-4"><DayPicker days={scrambleDays} activeDay={activeDay} onChange={setActiveDay} /></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{teams.map((team, teamIndex) => <div key={team.id} className="border border-[#bca062]/55 bg-black/15 p-4"><p className="border-b border-[#bca062]/45 pb-2 font-serif text-lg font-bold text-[#f1e6ce]">Team {teamIndex + 1}</p><div className="mt-3 space-y-2">{team.playerIds.map((id, slotIndex) => <select disabled={locked} aria-label={`Team ${teamIndex + 1} player ${slotIndex + 1}`} key={`${team.id}-${slotIndex}`} value={id} onChange={(event) => setTeams((current) => swapTeamPlayer(current, team.id, slotIndex, event.target.value))} className="field w-full text-sm">{players.map((player) => <option key={player.id} value={player.id}>{player.name} · {player.tier}</option>)}</select>)}</div></div>)}</div></section>
  </div>;
}
