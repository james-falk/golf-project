...
const SkinsTab: React.FC<SkinsTabProps> = ({ roundData, updateRoundData, isReadOnly = false }) => {
  ...
  useEffect(() => {
    ...
    const potPerSkin = skinsWon > 0 ? Math.round(totalPot / skinsWon) : 0; // Round to whole dollars
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="px-3 py-2 bg-green-50 border border-green-300 rounded-md text-green-900 font-semibold">
                              {skinResults.filter(skin => skin.winner).length} skins / ${potPerHole > 0 ? Math.round(potPerHole) : '0'}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
