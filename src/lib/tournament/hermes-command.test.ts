...
describe("Hermes scoring commands", () => {
  it("saves a complete player card by name", () => {
    const result = applyHermesScoringCommand(makeMockTournamentState(), { type: "player-card", day: "thursday", player: "Ethan", scores: card });
  ...
  it("corrects one player hole without replacing the remaining card", () => {
    const initial = applyHermesScoringCommand(makeMockTournamentState(), { type: "player-card", day: "friday", player: "Ethan", scores: card }).state;
  ...
  it("records a closest-to-pin winner", () => {
    const result = applyHermesScoringCommand(makeMockTournamentState(), { type: "ctp", day: "saturday", hole: 6, player: "James" });
    expect(result.state.closestToPin["saturday:6"]).toBe("player-10");
  });

  it("rejects malformed cards and Thursday scramble", () => {
    expect(() => applyHermesScoringCommand(makeMockTournamentState(), { type: "player-card", day: "thursday", player: "Ethan", scores: [4, 5] })).toThrow("exactly 18");
    expect(() => applyHermesScoringCommand(makeMockTournamentState(), { type: "round-status", day: "thursday", round: "scramble", status: "posted" })).toThrow("no Thursday scramble");
  });
});
